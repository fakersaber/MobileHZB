...
public:
	...
	virtual void RHIMapStagingSurfaceNoFlush(FRHITexture* Texture, FRHIGPUFence* Fence, void*& OutData) final override;
	virtual void RHIUnMapStagingSurfaceNoFlush(FRHITexture* Texture) final override;
	...
	FORCEINLINE void CachedSetupTextureStage(FOpenGLContextState& ContextState, GLint TextureIndex, GLenum Target, GLuint Resource, GLint BaseMip, GLint NumMips)
	{
		...
		// @StarLight code - BEGIN HZB, Created by yjh
		const bool bSameMipmap = (TextureState.LimitMip == BaseMip);
		// @StarLight code - END HZB


		if (bSameTarget && bSameResource && bSameMipmap)
		{
...
//#endif
	...
	virtual void* LockTexture2DArray_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture2DArray* Texture, uint32 TextureIndex, uint32 MipIndex, EResourceLockMode LockMode, uint32& DestStride, bool bLockWithinMiptail) final override;
	virtual void UnlockTexture2DArray_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture2DArray* Texture, uint32 TextureIndex, uint32 MipIndex, bool bLockWithinMiptail) final override;

...
void* FDynamicRHI::LockTexture2DArray_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture2DArray* Texture, uint32 TextureIndex, uint32 MipIndex, EResourceLockMode LockMode, uint32& DestStride, bool bLockWithinMiptail)
{
...
void FDynamicRHI::UnlockTexture2DArray_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture2DArray* Texture, uint32 TextureIndex, uint32 MipIndex, bool bLockWithinMiptail)
{
...
void FDynamicRHI::RHIMapStagingSurface_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture* Texture, FRHIGPUFence* Fence, void*& OutData, int32& OutWidth, int32& OutHeight)
{
	...
	UE_LOG(LogTemp, Log, TEXT("Fence->NumPendingWriteCommands.GetValue(): %d"), Fence->NumPendingWriteCommands.GetValue());
	if (Fence == nullptr || !Fence->Poll() || Fence->NumPendingWriteCommands.GetValue() != 0)
	{
		UE_LOG(LogTemp, Log, TEXT("Fence == nullptr: %b, !Fence->Poll(): %b, Value: %b"), Fence == nullptr, !Fence->Poll(), Fence->NumPendingWriteCommands.GetValue() != 0);
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_MapStagingSurface_Flush);
		RHICmdList.ImmediateFlush(EImmediateFlushType::FlushRHIThread);
	}
	UE_LOG(LogTemp, Log, TEXT("RHIMapStagingSurface_RenderThread: %u"), FPlatformTime::Cycles() - IdleStart);

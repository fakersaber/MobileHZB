...
void FMetalDynamicRHI::RHIMapStagingSurfaceNoFlush(FRHITexture* Texture, FRHIGPUFence* Fence, void*& OutData) {
	@autoreleasepool{
		FMetalGPUFence* MetalFence = ResourceCast(Fence);
		if (MetalFence && MetalFence->Wait(500)) {
...
void FMetalDynamicRHI::RHIUnMapStagingSurfaceNoFlush(FRHITexture* Texture) {
	@autoreleasepool{
		FMetalSurface* Surface = GetMetalSurfaceFromRHITexture(TextureRHI);

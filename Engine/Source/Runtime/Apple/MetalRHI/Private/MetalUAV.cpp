...
bool FMetalGPUFence::Wait(uint64 Millis) {
	@autoreleasepool{
		//mtlcpp wraps waitUntilData as Wait, it will Blocks the current thread until the condition is signaled or the specified time limit is reached.
		//So it has the same effect as polling
		return Fence.Wait(Millis);
	}
}
